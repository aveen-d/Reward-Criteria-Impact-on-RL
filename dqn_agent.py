...
class DQNAgent(Node):
    def __init__(self, stage):
        ...
        ************************************************************"""
        ...
        self.state_size = 28
        self.action_size = 5
        self.episode_size = 3000 # task specific
        ...
        # Replay memory
        self.memory = collections.deque(maxlen=10000000)

        # Build model and target model
        self.model = self.build_model()
        self.target_model = self.build_model()
        
        self.update_target_model_start = 64
        self.logdir = "./logs1/scalars/" + datetime.now().strftime("%Y%m%d-%H%M%S")
        ...
        self.model_path = os.path.join(
            self.model_dir_path,
            'R2_stage_'+str(self.stage)+'_episode'+str(self.load_episode)+'.h5')

        if self.load_model:
            self.model.set_weights(load_model(self.model_path).get_weights())
            with open(os.path.join(
                    self.model_dir_path,
                    'R2_stage_'+str(self.stage)+'_episode'+str(self.load_episode)+'.json')) as outfile:
    ...
    *******************************************************************************"""
    def process(self):
        ...
        for episode in range(self.load_episode+1, self.episode_size):
                ...
                if local_step > 1:
                    ...
                    if done:
                        ...
                        print(
                            ...
                            "glbal step: ",global_step)
                        tf.summary.scalar('reward per episode',data=score,step=episode)
                        tf.summary.scalar('epsilon per episode',data=self.epsilon,step=episode)
                        if len(self.memory) > self.train_start:
                        
                            tf.summary.scalar('loss per episode',data=numpy.average(numpy.asarray(loss)),step=episode)
                        param_keys = ['epsilon']
            ...
            if episode % 10 == 0:
                self.model_path = os.path.join(
                    self.model_dir_path,
                    'R3_stage_'+str(self.stage)+'_episode'+str(episode)+'.h5')
                self.model.save(self.model_path)
                with open(os.path.join(
                    self.model_dir_path,
                        'R3_stage_'+str(self.stage)+'_episode'+str(episode)+'.json'), 'w') as outfile:
    ...
    def build_model(self):
        ...
        model.add(Dropout(0.2))
        model.add(Dense(self.action_size, kernel_initializer='lecun_uniform')) # Q-value prediction
    ...
    def get_action(self, state): 
        if numpy.random.random() <= self.epsilon: #pick a value between [0,1] and if it is less than epsilon pick a random action else pick exploited action
    ...
    def train_model(self, target_train_start=False):
        ...
        '''
        sample = random.sample(self.memory, 64)
        state, action, reward, next_state, done = map(np.asarray, zip(*sample))
        y_batch=self.target_model.predict(state)
        self.max_q_value=numpy.max(y_batch,axis=1)
        target_value=self.target_model.predict(next_state).max(axis=1)
        y_batch[range(64),action]= reward + (1-done) * target_value * self.discount_factor     
        hist_=self.model.fit(state, y_batch, batch_size=self.batch_size, epochs=1, verbose=0)
        return hist_.history['loss']
